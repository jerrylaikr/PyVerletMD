...
R_1 = 7
R_C = 7.5
MASS = 64/1000/(6.02214076e23)*6.242e22 # 1[kg]/atom = 6.242e22[eV*A^2*ps^-2]/atom
...
vel_ini = np.array([-1.98, -1.24, -2.38, -2.02, 3.08, 2.52, 2.64, -3.0, -2.6, 1.74, 1.24, 2.0])
...
def animate(frame_idx):
    ...
    time_text.set_text("frame = {}, time = {:.2f}ps / {:.2f}ps".format(frame_idx,frame_idx*dt,n_steps*dt))
    ...
    for i in range(n_atoms):
        ...
        annot.append(ax.text(x_mark[i], y_mark[i]+0.4, annot_text, color="c", fontsize=8))
...
class Atom():
    ...
    def update_dist(self):
        for j in range(n_atoms):
            ...
            self.dist[j] = (self.pos - self.atoms_list[j].pos) - BOX * np.round((self.pos - self.atoms_list[j].pos)/BOX)
        # print("dist array={}".format(self.dist)) # FOR TESTING

    def evaluate_force(self):
        r_ij_vec = np.zeros(2)
        r_ij_mag = 0
        force_temp = np.array([0., 0.])
        for j in range(n_atoms):
            # self is atoms[i], interacting with atoms[j]
            r_ij_vec = self.dist[j]
            r_ij_mag = np.linalg.norm(r_ij_vec)
            if (r_ij_mag > 0) and (r_ij_mag <= R_1):
                force_temp += -(-38608.67 * r_ij_mag**(-13) + 68.744 * r_ij_mag**(-6)) * (r_ij_vec / r_ij_mag)
            elif (r_ij_mag > R_1) and (r_ij_mag < R_C):
                force_temp += -( - 0.0036 * (r_ij_mag - R_C)**2 - 0.0001994 * (r_ij_mag - R_C)) * (r_ij_vec / r_ij_mag)
        return force_temp
...
for i in range(n_atoms):
    atoms[i].update_dist()
    pos_big_array[0,2*i:2*i+2] = atoms[i].pos - atoms[i].vel * dt - (dt**2/2) * atoms[i].acc
    pos_big_array[0,2*i:2*i+2] = pos_big_array[0,2*i:2*i+2] % BOX # Periodic Boundary
